...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        /** TODO: Your code here. */
        while (parentIndex(index) != 0 && contents[index].priority() < contents[parentIndex(index)].priority()) {
    ...
    private void sink(int index) {
        ...
        /** TODO: Your code here. */
        int minNode = min(leftIndex(index), rightIndex(index));
        while (minNode != 0 && contents[index].priority() >= contents[minNode].priority()) {
            swap(index, minNode);
            index = minNode;
            minNode = min(leftIndex(index), rightIndex(index));
        }
    }
